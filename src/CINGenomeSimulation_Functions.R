...
## For the example: "2_Example_Calculate_activities.R"
calcSigActivities = function(CXS, SIGS, BASENAME, SIGNAMES) {
  
  ## Load files
  V = CXS
  W = readRDS(SIGS)
  
  # Sanity check signature matrices
  ...
  # Rename signatures
  colnames(W) = SIGNAMES[ match(colnames(W), names(SIGNAMES)) ]
  W=W[,SIGNAMES]
  ...
  saveRDS(object = Hraw, file = paste0(BASENAME, "_raw.rds"))
  
  H = t( apply(Hraw, 2, function(x) x/sum(x)) )
  write.table(x = H, file = paste0(BASENAME, ".txt"), quote = FALSE, row.names = TRUE, col.names = TRUE, sep = "\t")
  saveRDS(object = H, file = paste0(BASENAME, ".rds"))
